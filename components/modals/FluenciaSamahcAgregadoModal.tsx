...
import { Escola, RegistroFluenciaSamahcAgregado } from '../../types';
import { Save, ClipboardCheck, Users, GraduationCap, TrendingUp, X, Calendar, Layers } from 'lucide-react';
import { Button } from '../ui/Button';

interface FluenciaSamahcAgregadoModalProps {
    isOpen: boolean;
    onClose: () => void;
    escola: Escola;
    onSave: (registro: Omit<RegistroFluenciaSamahcAgregado, 'id' | 'escolaId' | 'dataRegistro' | 'responsavel'> & { id?: string }) => void;
    onDelete: (id: string) => void;
}

export const FluenciaSamahcAgregadoModal: React.FC<FluenciaSamahcAgregadoModalProps> = ({
    isOpen,
    onClose,
    escola,
    onSave,
    ...
    const [formData, setFormData] = useState({
        ...
        edicao: 'Diagnóstica' as 'Entrada' | 'Saída' | 'Diagnóstica' | 'Formativa' | 'Somativa',
        etapaAplicacao: '',
    ...
    useEffect(() => {
        const sum =
            ...
            Number(formData.preLeitor) +
            Number(formData.naoLeitor) +
            Number(formData.naoAvaliado);
        ...
        setTimeout(() => {
            const registro = {
                ...
                classificacao: {
                    ...
                    preLeitor: Number(formData.preLeitor),
                    naoLeitor: Number(formData.naoLeitor),
                    naoAvaliado: Number(formData.naoAvaliado)
                }
            };
            onSave(registro);
            setLoading(false);
    ...
    const handleEdit = (reg: RegistroFluenciaSamahcAgregado) => {
        setEditingId(reg.id);
        setFormData({
            polo: reg.polo,
            ano: reg.ano,
            ...
            preLeitor: reg.classificacao.preLeitor,
            naoLeitor: reg.classificacao.naoLeitor,
            naoAvaliado: reg.classificacao.naoAvaliado,
        });
                ...
                <div className="relative overflow-hidden bg-slate-900 px-5 py-5 leading-tight">
                    ...
                    <div className="relative z-10 flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <h2 className="text-xl font-black text-white tracking-tight">Registro de Fluência SAMAHC</h2>
                ...
                <form onSubmit={handleSubmit} className="p-5 space-y-5 max-h-[75vh] overflow-y-auto custom-scrollbar shadow-inner bg-slate-50/10">
                    ...
                    <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm space-y-5">
                        ...
                        <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
                            {[
                                ...
                                { label: 'Pré-leitor', name: 'preLeitor', color: 'text-amber-600', border: 'border-amber-100', bg: 'bg-amber-50/50' },
                                { label: 'Não Leitor', name: 'naoLeitor', color: 'text-rose-600', border: 'border-rose-100', bg: 'bg-rose-50/50' },
                                { label: 'Não Avaliado', name: 'naoAvaliado', color: 'text-slate-600', border: 'border-slate-100', bg: 'bg-slate-50/50' },
                            ].map((item) => (
                    ...
                    <div className="flex justify-end gap-3 pt-4 pb-8 border-b border-slate-100">
                        ...
                        <Button type="submit" variant="success" disabled={loading} icon={Save} isLoading={loading}>
                            {loading ? 'Salvando...' : editingId ? 'Atualizar Registro' : 'Confirmar Registro'}
                        </Button>
                    </div>
                        ...
                        <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm">
                            <table className="w-full text-left text-sm">
                                ...
                                <tbody className="divide-y divide-slate-50">
                                    {escola.dadosEducacionais.registrosFluenciaSamahcAgregados && escola.dadosEducacionais.registrosFluenciaSamahcAgregados.length > 0 ? (
                                        escola.dadosEducacionais.registrosFluenciaSamahcAgregados
                                            .sort((a, b) => b.ano - a.ano || (a.edicao === 'Diagnóstica' ? -1 : 1))
                                            .map((reg) => (
