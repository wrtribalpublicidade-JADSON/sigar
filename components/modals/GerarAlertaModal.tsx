...
import { 
  X, Send, Calendar, AlertTriangle, Clock, CheckCircle, ShieldAlert, 
  Sparkles, School, Users, Layers, Printer, FileText, CheckCircle2 
} from 'lucide-react';
...
export const GerarAlertaModal: React.FC<GerarAlertaModalProps> = ({
  ...
  const getDestinatarioPreview = () => {
    if (items.length === 0) return '';
    const item = items[0];
    if (destinatario === 'RESPONSAVEL_DIRETO') {
      return `${item.usuario_nome} (${item.usuario_perfil || 'Docente'})`;
    }
    if (destinatario === 'GESTAO_ESCOLAR') {
      return `Gestão Escolar: ${item.co_responsaveis_nomes || 'Coordenadores e Gestores da Unidade'}`;
    }
    return `${item.usuario_nome} e Gestão Escolar (${item.co_responsaveis_nomes || 'Coordenadores da Unidade'})`;
  };
  ...
  // Preview items with updated deadline and observation for print
  const itemsForPrint: AlertaPendencia[] = items.map(item => ({
    ...item,
    prazo,
    prioridade,
    observacao_alerta: observacao || item.observacao_alerta,
    ...
    <>
      <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fade-in">
        <div className="bg-white rounded-3xl w-full max-w-xl shadow-2xl overflow-hidden animate-scale-in border border-slate-200 max-h-[90vh] flex flex-col">
                ...
                <div>
                  <h3 className="text-xl font-black tracking-tight">
                    {sentSuccess 
                      ...
                      : isBatch 
                        ? `Gerar Alertas em Massa (${items.length} itens)` 
                        : 'Gerar Alerta e Notificação'}
                  </h3>
          ...
          {/* SUCESSO: OPÇÃO DE IMPRESSÃO / SALVAR EM PDF */}
          {sentSuccess ? (
            <div className="p-8 text-center space-y-6 flex-1 overflow-y-auto">
              ...
              <div>
                <h4 className="text-lg font-black text-slate-800">
                  Alerta e Notificação Enviado!
                </h4>
                <p className="text-xs text-slate-600 font-medium mt-1.5 max-w-md mx-auto leading-relaxed">
                  O histórico de envio do usuário notificado foi registrado com sucesso. 
                  O popup de alerta será exibido exclusivamente ao destinatário correspondente quando acessar o sistema.
                </p>
              </div>

              {/* Card Resumo do Notificado */}
              <div className="bg-slate-50 border border-slate-200 rounded-2xl p-4 text-left text-xs space-y-2 max-w-md mx-auto">
                <div className="flex justify-between items-center pb-2 border-b border-slate-200">
                  <span className="text-slate-400 font-bold uppercase text-[10px]">Destinatário Notificado</span>
                  <span className="font-bold text-slate-800 text-right max-w-[240px] truncate">
                    {isBatch ? `${items.length} servidores notificados` : getDestinatarioPreview()}
                  </span>
                </div>
                <div className="flex justify-between items-center pb-2 border-b border-slate-200">
                ...
                <div className="flex justify-between items-center">
                  <span className="text-slate-400 font-bold uppercase text-[10px]">Prioridade</span>
                  <span className="font-black text-slate-800 uppercase">{prioridade}</span>
                </div>
              </div>
              ...
              <div className="pt-4 flex flex-col sm:flex-row items-center justify-center gap-3">
                <button
                  type="button"
                  onClick={handleFinishAndClose}
                  className="w-full sm:w-auto px-6 py-2.5 rounded-xl text-xs font-bold text-slate-600 hover:bg-slate-100 transition-colors uppercase tracking-wider"
                >
                  Concluir e Fechar
                </button>
                ...
                <button
                  ...
                  className="w-full sm:w-auto px-6 py-2.5 rounded-xl text-xs font-black text-white bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 shadow-lg shadow-orange-500/25 flex items-center justify-center gap-2 uppercase tracking-wider transition-all"
                >
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-5 overflow-y-auto custom-scrollbar flex-1">
              ...
              <div className="bg-gradient-to-br from-orange-50/60 to-slate-50 border border-orange-200/80 rounded-2xl p-4 space-y-2.5">
                ...
                {/* Destinatário Notificado Preview */}
                <div className="bg-orange-50/80 border border-orange-200 rounded-xl p-3 flex items-start gap-2 text-xs text-orange-950 mt-2">
                  <CheckCircle className="w-4 h-4 text-orange-600 shrink-0 mt-0.5" />
                  <div>
                    <span className="font-bold block uppercase text-[10px] tracking-wider text-orange-800">
                      Usuário Notificado (Receberá o Alerta):
                    </span>
                    <span className="font-semibold text-slate-800">
                      {isBatch ? `${items.length} servidores/equipes selecionadas` : getDestinatarioPreview()}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="pt-2 flex items-center justify-between gap-3 border-t border-slate-100">
                <button
                  type="button"
                  onClick={() => setIsPrintOpen(true)}
                  className="px-4 py-2.5 rounded-xl text-xs font-bold text-slate-700 bg-slate-100 hover:bg-slate-200 border border-slate-200 transition-colors flex items-center gap-1.5 uppercase tracking-wider"
                  title="Visualizar Notificação e Salvar em PDF"
                >
      ...
      {/* Visualização de Impressão / Salvar em PDF */}
      {isPrintOpen && (
        <PrintableAlertasPendenciasReport
          items={itemsForPrint}
          initialMode="oficio"
          escolaNome={items[0]?.escola_nome}
