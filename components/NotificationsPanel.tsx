...
import { Escola, Coordenador, ViewState } from '../types';
import {
...
export const NotificationsPanel: React.FC<NotificationsPanelProps> = ({ 
    ...
    const [activeTab, setActiveTab] = useState<'ALERTAS_PENDENCIAS' | 'NOTIFICACOES_SISTEMA'>('ALERTAS_PENDENCIAS');
    ...
    return (
        <div className="space-y-6 pb-12 animate-fade-in">
            <PageHeader
                ...
                subtitle="Acompanhamento automatizado de demandas, prazos e pendências operacionais"
                icon={Bell}
                badgeText="MONITORAMENTO"
            />

            {/* TAB SELECTOR */}
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-1.5 flex gap-1.5 w-fit">
                <button
                    onClick={() => setActiveTab('ALERTAS_PENDENCIAS')}
                    className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-xs font-black uppercase tracking-wider transition-all ${
                        activeTab === 'ALERTAS_PENDENCIAS'
                            ? 'bg-slate-900 text-white shadow-md'
                            : 'text-slate-500 hover:text-slate-900 hover:bg-slate-50'
                    }`}
                >
                    <ShieldAlert className={`w-4 h-4 ${activeTab === 'ALERTAS_PENDENCIAS' ? 'text-orange-400' : ''}`} />
                ...
                <button
                    onClick={() => setActiveTab('NOTIFICACOES_SISTEMA')}
                    className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-xs font-black uppercase tracking-wider transition-all ${
                        activeTab === 'NOTIFICACOES_SISTEMA'
                            ? 'bg-slate-900 text-white shadow-md'
                            : 'text-slate-500 hover:text-slate-900 hover:bg-slate-50'
                    }`}
                >
                    <Bell className={`w-4 h-4 ${activeTab === 'NOTIFICACOES_SISTEMA' ? 'text-orange-400' : ''}`} />
                    <span>Notificações das Unidades ({pendingData.length})</span>
                </button>
            </div>

            {/* TAB CONTENT: ALERTAS DE PENDÊNCIAS */}
            {activeTab === 'ALERTAS_PENDENCIAS' && (
                <AlertasPendenciasTab
                    escolas={escolas}
            ...
            {/* TAB CONTENT: NOTIFICAÇÕES DO SISTEMA (EXISTENTES) */}
            {activeTab === 'NOTIFICACOES_SISTEMA' && (
