...
interface ProfessorComEscola extends RecursoHumano {
    escolaNome: string;
    escolaId: string;
    escolaLocalizacao: string;
}

interface PrintableProfessoresReportProps {
    professores: ProfessorComEscola[];
    filtroVinculo: string;
    filtroEtapa: string;
    filtroAnoSerie?: string;
    filtroComponente: string;
    filtroEscola: string;
}

export const PrintableProfessoresReport: React.FC<PrintableProfessoresReportProps> = ({
    professores,
    filtroVinculo,
    filtroEtapa,
    filtroAnoSerie,
    ...
    const efetivos = professores.filter(s => s.tipoVinculo === 'Efetivo').length;
    const contratados = professores.filter(s => s.tipoVinculo === 'Contratado').length;
    const permutados = professores.filter(s => s.tipoVinculo === 'Permutado').length;
    const totalEscolas = new Set(professores.map(s => s.escolaNome)).size;
    const totalEtapas = new Set(professores.map(s => s.etapaAtuacao).filter(Boolean)).size;

    const filtroDescricao = [
        filtroVinculo !== 'Todos' ? `Vínculo: ${filtroVinculo}` : null,
        filtroEtapa !== 'Todas' ? `Etapa: ${filtroEtapa}` : null,
        filtroAnoSerie && filtroAnoSerie !== 'Todos' ? `Ano/Série: ${filtroAnoSerie}` : null,
        filtroComponente !== 'Todos' ? `Componente: ${filtroComponente}` : null,
        filtroEscola !== 'Todas' ? `Escola: ${filtroEscola}` : null,
    ].filter(Boolean).join(' • ') || 'Todos os Professores';

    return createPortal(
        <div id="print-professores-report" className="hidden print:block bg-white text-slate-900" style={{ fontFamily: "'Inter', 'Segoe UI', system-ui, sans-serif" }}>
            ...
            <div className="text-center mb-3 pb-3" style={{ borderBottom: '2pt solid #0f172a' }}>
                ...
                <div style={{ width: '60pt', height: '1.5pt', background: '#2563eb', margin: '0 auto 6pt' }} />
                ...
                <p style={{ fontSize: '8pt', fontWeight: 700, color: '#64748b', letterSpacing: '0.15em', textTransform: 'uppercase' }}>
                    Corpo Docente — Rede Municipal de Ensino
                </p>
            </div>
            ...
            <div className="print-avoid-break" style={{ marginBottom: '14pt', display: 'flex', gap: '8pt' }}>
                <div style={{ flex: 1, padding: '8pt', border: '0.5pt solid #e2e8f0', background: '#f1f5f9' }}>
                    <p style={{ fontSize: '7pt', fontWeight: 800, color: '#64748b', textTransform: 'uppercase', marginBottom: '2pt' }}>Total de Professores</p>
                    <p style={{ fontSize: '14pt', fontWeight: 900, color: '#0f172a' }}>{professores.length}</p>
                </div>
                ...
                <div style={{ flex: 1, padding: '8pt', border: '0.5pt solid #e2e8f0', background: '#f5f3ff' }}>
                    <p style={{ fontSize: '7pt', fontWeight: 800, color: '#7c3aed', textTransform: 'uppercase', marginBottom: '2pt' }}>Etapas</p>
                    <p style={{ fontSize: '14pt', fontWeight: 900, color: '#5b21b6' }}>{totalEtapas}</p>
                </div>
            </div>

            {/* ====== TABLE ====== */}
            <div style={{ marginBottom: '10pt' }}>
                <div style={{ fontSize: '8pt', fontWeight: 900, textTransform: 'uppercase', letterSpacing: '0.2em', background: '#1e3a8a', color: '#fff', padding: '5pt 10pt' }}>
                    Relação Nominal — Corpo Docente
                </div>
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                    <thead>
                        <tr>
                            <th style={{ padding: '5pt 6pt', border: '0.5pt solid #e2e8f0', fontSize: '6.5pt', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.1em', color: '#64748b', background: '#f8fafc', textAlign: 'center', width: '3%' }}>Nº</th>
                            <th style={{ padding: '5pt 6pt', border: '0.5pt solid #e2e8f0', fontSize: '6.5pt', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.1em', color: '#64748b', background: '#f8fafc', textAlign: 'left' }}>Nome do Professor</th>
                            ...
                            <th style={{ padding: '5pt 6pt', border: '0.5pt solid #e2e8f0', fontSize: '6.5pt', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.1em', color: '#64748b', background: '#f8fafc', textAlign: 'left' }}>Etapa de Atuação</th>
                            <th style={{ padding: '5pt 6pt', border: '0.5pt solid #e2e8f0', fontSize: '6.5pt', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.1em', color: '#64748b', background: '#f8fafc', textAlign: 'left' }}>Componente Curricular</th>
                            <th style={{ padding: '5pt 6pt', border: '0.5pt solid #e2e8f0', fontSize: '6.5pt', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.1em', color: '#64748b', background: '#f8fafc', textAlign: 'center', width: '8%' }}>Vínculo</th>
                            <th style={{ padding: '5pt 6pt', border: '0.5pt solid #e2e8f0', fontSize: '6.5pt', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.1em', color: '#64748b', background: '#f8fafc', textAlign: 'center', width: '6%' }}>C.H.</th>
                            <th style={{ padding: '5pt 6pt', border: '0.5pt solid #e2e8f0', fontSize: '6.5pt', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.1em', color: '#64748b', background: '#f8fafc', textAlign: 'center', width: '9%' }}>Nomeação</th>
                        </tr>
                    </thead>
                    <tbody>
                        {professores.map((prof, index) => (
                            <tr key={`${prof.id}-${prof.escolaNome}-${index}`}>
                                ...
                                <td style={{ padding: '3pt 6pt', border: '0.5pt solid #e2e8f0', fontSize: '8pt', fontWeight: 700, color: '#334155' }}>
                                    {prof.nome}
                                    {prof.email && <div style={{ fontSize: '6.5pt', fontWeight: 400, color: '#64748b', marginTop: '1pt' }}>{prof.email}</div>}
                                    {prof.telefone && <div style={{ fontSize: '6.5pt', fontWeight: 400, color: '#94a3b8', marginTop: '1pt' }}>{prof.telefone}</div>}
                                </td>
                                <td style={{ padding: '3pt 6pt', border: '0.5pt solid #e2e8f0', fontSize: '7.5pt', fontWeight: 600, color: '#475569' }}>
                                    {prof.escolaNome}
                                    <div style={{ fontSize: '6pt', color: '#94a3b8', textTransform: 'uppercase', fontWeight: 700 }}>{prof.escolaLocalizacao}</div>
                                </td>
                                <td style={{ padding: '3pt 6pt', border: '0.5pt solid #e2e8f0', fontSize: '7.5pt', fontWeight: 600, color: '#475569' }}>
                                    {prof.etapaAtuacao || '-'}
                                    {prof.modalidadeInfantil && prof.modalidadeInfantil.length > 0 && (
                                        <div style={{ fontSize: '6pt', color: '#2563eb', marginTop: '1pt', fontWeight: 700 }}>{prof.modalidadeInfantil.join(', ')}</div>
                                    )}
                                    {prof.anosIniciaisAtuacao && prof.anosIniciaisAtuacao.length > 0 && (
                                        <div style={{ fontSize: '6pt', color: '#059669', marginTop: '1pt', fontWeight: 700 }}>{prof.anosIniciaisAtuacao.join(', ')}</div>
                                    )}
                                </td>
                                <td style={{ padding: '3pt 6pt', border: '0.5pt solid #e2e8f0', fontSize: '7.5pt', fontWeight: 600, color: '#475569' }}>
                                    {prof.componenteCurricular || '-'}
                                </td>
                        ...
                        {professores.length === 0 && (
                            <tr>
                                <td colSpan={8} style={{ padding: '10pt', textAlign: 'center', color: '#94a3b8', fontStyle: 'italic', fontSize: '9pt' }}>
            ...
            <div className="print-footer" style={{ marginTop: '30pt', display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: '6.5pt', fontWeight: 600, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.3em', borderTop: '0.5pt solid #e2e8f0', paddingTop: '6pt' }}>
                ...
                <span>Controle de Professores • {currentYear}</span>
            </div>
        </div>,
        document.body
    );
